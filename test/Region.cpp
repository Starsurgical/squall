...
TEST_CASE("SRgnCombineRecti", "[region]") {
    ...
    SECTION("OR operation combines rects") {
        ...
        // └─────────┘
        CHECK(numRects == 3);
        CHECK_THAT(buffer[0], MatchesRecti({ 0, 0, 10, 6 }));
        CHECK_THAT(buffer[1], MatchesRecti({ 0, 6, 18, 10 }));
        CHECK_THAT(buffer[2], MatchesRecti({ 5, 10, 18, 14 }));
    }
    ...
    SECTION("OR operation on self does nothing") {
        ...
        CHECK(numRects == 1);
        CHECK_THAT(buffer[0], MatchesRecti({ 0, 0, 10, 10 }));
    }
    ...
    SECTION("OR operation merges vertical rects") {
        ...
        CHECK(numRects == 1);
        CHECK_THAT(buffer[0], MatchesRecti({ 0, 0, 10, 30 }));
    }
    ...
    SECTION("OR operation merges horizontal rects") {
        ...
        CHECK(numRects == 1);
        CHECK_THAT(buffer[0], MatchesRecti({ 0, 0, 30, 10 }));
    }
    ...
    SECTION("AND operation intersects rects") {
        ...
        // └─────────┘
        CHECK(numRects == 1);
        CHECK_THAT(buffer[0], MatchesRecti({ 5, 6, 10, 10 }));
    }
    ...
    SECTION("XOR operation takes exclusive differences of rects") {
        ...
        CHECK(numRects == 4);
        CHECK_THAT(buffer[0], MatchesRecti({ 0, 0, 10, 6 }));
        CHECK_THAT(buffer[1], MatchesRecti({ 0, 6, 5, 10 }));
        CHECK_THAT(buffer[2], MatchesRecti({ 10, 6, 18, 10 }));
        CHECK_THAT(buffer[3], MatchesRecti({ 5, 10, 18, 14 }));
    }
    ...
    SECTION("COPY operation splits intersecting rects") {
        ...
        CHECK(numRects == 2);
        CHECK_THAT(buffer[0], MatchesRecti({ 0, 0, 10, 6 }));
        CHECK_THAT(buffer[1], MatchesRecti({ 0, 6, 10, 10 }));
    }
    ...
    SECTION("COPY operation splits intersecting rects 2") {
        ...
        // └───────────┘
        CHECK(numRects == 3);
        CHECK_THAT(buffer[0], MatchesRecti({ 0, 0, 10, 4 }));
        CHECK_THAT(buffer[1], MatchesRecti({ 0, 4, 10, 6 }));
        CHECK_THAT(buffer[2], MatchesRecti({ 0, 6, 10, 10 }));
    }
    ...
    SECTION("operation doesn't work when operating rect width or height is <= 0") {
        ...
        SRgnGetRectsi(region, &numRects, buffer);
        CHECK(numRects == 1);
        CHECK_THAT(buffer[0], MatchesRecti(baseRect));
...
TEST_CASE("SRgnGetBoundingRecti", "[region]") {
    ...
    SECTION("returns invalid rect when using an invalid region object") {
        ...
        CHECK_THAT(result, MatchesRecti({ LONG_MIN, LONG_MIN, 0, 0 }));
    }
...
TEST_CASE("SRgnIsPointInRegioni", "[region]") {
    ...
    SECTION("false if region has no rects") {
        auto x = GENERATE(0, 1, LONG_MAX);
        auto y = GENERATE(0, -1, LONG_MIN);
