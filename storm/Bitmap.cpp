...
int32_t SBmpDecodeImage(int32_t imagetype, void* imagedata, uint32_t imagebytes, STORM_PALETTEENTRY* paletteentries, uint8_t* bitmapbits, uint32_t buffersize, uint32_t* width, uint32_t* height, uint32_t* bitdepth) {
    if (width) *width = 0;
    if (height) *height = 0;
    ...
    if (imagetype == SBMP_IMAGETYPE_AUTO && imagebytes >= 2) {
        imagetype = DetermineImageType(*static_cast<uint16_t*>(imagedata));
    }
...
int32_t SBmpGetAligned(int32_t width, int32_t align) {
    if (width % align == 0) {
        return width;
    }
...
int32_t SBmpRealign(void* imagedata, uint32_t imagebytes, int32_t width, int32_t height, int32_t alignment, int32_t* out_newwidth) {
    int32_t newwidth = SBmpGetAligned(width, alignment);
    if (newwidth == width) {
        return 1;
    }

    uint8_t* newptr = static_cast<uint8_t*>(imagedata);
